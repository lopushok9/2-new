...
async function identifyImageWithINat(imageBuffer, format = 'markdown') {
  ...
  if (!topResult) {
    ...
    const header = (format === 'markdown') ? `### ${commonName}\n` : `${commonName}\n`;
    ...
    if (taxonImage) {
      let imageLine;
      if (format === 'markdown') {
        // For the website, embed the image
        imageLine = `![Image of ${commonName}](${taxonImage})\n\n`;
      } else { // 'plain' format for the bot
        // For the bot, create a hyperlink
        imageLine = `[Image of ${commonName}](${taxonImage})\n\n`;
      }
      formattedContent += imageLine;
    }
  }
